...
class MemoryRetrievalRouter:
    ...
    def _llm_classify(self, query: str, rule_result: list) -> Optional[List[str]]:
        ...
        prompt = (
            "你是一个记忆检索路由器。根据用户消息，判断需要检索哪些记忆层。\n\n"
            "【三层记忆定义】\n"
    ...
    def _analyze_score_distribution(self, sorted_layers: list) -> dict:
        ...
        返回:
          spread: 三层得分极差 (0~1)
          pattern: "concentrated"(集中单层) | "dual"(双峰) | "diffuse"(均匀)
          single_dominant: 是否单层显著领先
        """
        scores = [s for _, s in sorted_layers]
        spread = scores[0] - scores[-1] if len(scores) >= 3 else 1.0

        if spread <= self.CROSS_LAYER_RESONANCE_SPREAD:
            pattern = "diffuse"       # 三层得分接近 → 查询涉及多个维度
        elif len(sorted_layers) >= 2 and (scores[0] - scores[1]) <= self.AMBIGUITY_GAP:
    ...
    def decide(self, query: str, query_embedding: List[float],
        ...
        if conversation_rounds >= self.CONVERSATION_DEEP_PRIOR:
            threshold = self.ACTIVATION_THRESHOLD_DEEP
        ...
        # === 因子5: LLM 兜底触发检测 ===
        # 仅在得分分布高度不确定时触发，不再基于对话轮数触发
        need_llm = False
        llm_reason = ""

        if top_score < self.LLM_FALLBACK_MIN_SCORE:
            # 最高分都低于0.15 → 规则路由不自信
            need_llm = True
            llm_reason = f"置信度过低(max={top_score:.3f}<{self.LLM_FALLBACK_MIN_SCORE})"
        elif distribution["pattern"] == "diffuse" and top_score < self.DIFFUSE_LLM_TOP_THRESHOLD:
            # 均匀分布+最高分也不高 → 模型自己也不确定维度
            need_llm = True
